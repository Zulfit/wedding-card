...
import ParentsInvitation from "./ParentsInvitation";
...
export default function InvitationShell({ wedding, guestName }: InvitationShellProps) {
  ...
  return (
    <>
      ...
      <main className={`pb-16 ${gateState !== "hidden" ? "overflow-hidden h-dvh" : ""}`}>
        <Hero
          bride={wedding.bride}
          groom={wedding.groom}
        ...
        <ParentsInvitation
          ...
          groom={wedding.groom}
          date={wedding.date}
          invitation={wedding.parentsInvitation}
        />
      ...
      {isOpened && (
        <>
          ...
          <button
            type="button"
            onClick={toggleMusic}
            className={`fixed bottom-[4.5rem] right-4 z-50 p-2.5 bg-white/90 backdrop-blur-md rounded-full shadow-lg shadow-black/10 text-[#4a453e] hover:bg-white transition-all ${!isPlaying ? "ring-2 ring-[#d2c9bd] ring-offset-1" : ""}`}
            aria-label={isPlaying ? "Pause music" : "Play music"}
            title={isPlaying ? "Pause music" : "Tap to play music"}
          >
            {isPlaying ? (
